...
# Created: 2012-08-26 22:09:23 +0200
# Last modified: 2015-05-17 11:52:02 +0200
#
...
def readkey(name):
    """Read the data from a key file and count how often each byte value
    occurs.
    ...
    Returns:
        2-tuple of a byte-array and its counts.
    """
    with open(name, 'rb') as f:
        data = f.read()
        data = data.replace(b' ', b'').replace(b'\n', b'')
        ba = bytearray(base64.b64decode(data))
    counts = [0]*256
    for b in ba:
        counts[b] += 1
    return ba, counts
